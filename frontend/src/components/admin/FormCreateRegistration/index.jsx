...
function FormCreateRegistration() {
  ...
  const { handleSubmit, createRegistration, errors, register, setValue, infoForCreate, searchParams, setSearchParams, user, watch } = useFormCreateRegistration()
  const [open, setOpen] = useState(false)

  useEffect(() => {
    if (user) setValue('student', user)
                  ...
                  {infoForCreate?.students.map((student) => (
                    ...
                    >
                      ...
                      <CheckIcon
                        className={cn(
                          "ml-auto h-4 w-4",
                          user == student.user ? "opacity-100" : "opacity-0"
                        )}
                      />
                    </CommandItem>
                  ))}
          ...
          <Select {...register('classroom')} onValueChange={(value) => setValue('classroom', value)}>
            ...
            <SelectContent>
              <SelectGroup>
                ...
                {
                  // se não tiver nenhuma turma disponível para o livro selecionado
